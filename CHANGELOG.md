...
- **Oceans and coastlines now render as water.** Map areas with
  `natural=coastline` arcs (open sea, bays, tidal water) were previously
  ignored and rendered as land; the shoreline direction is now resolved per
...
- **Stale map caches are invalidated.** Cached maps now carry a processor
  version stamp; entries generated by an older processor (including maps
